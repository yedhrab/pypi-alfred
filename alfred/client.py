...
class AlfredClient:
                    ...
                    if response.status != 200:
                        ...
                        if version.parse(latest_version) > version.parse(current_version):
                            self.add_result(
                                ...
                                icon_path="alfred/icons/update.png",
                                arg=latest_release["html_url"]
                            )
                            plist["needupdate"] = True
                        plist["latestversion"] = latest_version
                        plist["latestrelease"] = latest_release["html_url"]
    ...
    async def update_with_git(
        self,
        remote: str,
        update_icon: str = "alfred/icons/updated.png",
        failed_icon: str = "alfred/icons/failed.png",
    ):
        process = await create_subprocess_exec("bash", "alfred/update.sh", remote, stdout=PIPE, stderr=PIPE)
        stdout, stderr = await process.communicate()
        output, error = stdout.decode("utf-8").strip(), stderr.decode("utf-8").strip()
        if error:
            self.add_result(title="Update failed", subtitle=error, icon_path=failed_icon)
        elif output != "Already up to date.":
            self.add_result(title="Update succeeded", subtitle=output, icon_path=update_icon)
